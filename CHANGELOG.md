...
## [1.9.1](https://github.com/ZA12A230/lx-netease-music-mobile/compare/v1.9.0...v1.9.1) - 2026-07-27

### V3 批次创新功能（13项）

- 协同过滤推荐：基于用户行为相似度推荐歌曲，发现更多喜欢的音乐
- 智能歌单整理：按歌手、风格、情绪、年代等多维度自动分类整理歌单
- 音乐发现雷达：多维度发现新音乐（趋势、季节、时段、小众宝藏等）
- 歌词多语言翻译：支持10种语言AI翻译，附带词汇注释和文化背景
- 音乐时光机：回顾X年前的今天你听过的音乐，重拾美好回忆
- 智能睡眠定时：90分钟睡眠周期、淡出、歌曲结束等多种智能模式
- 冥想模式：呼吸引导、专注计时，6种冥想类型、5种呼吸模式
- BPM跑步模式：根据步频匹配BPM，6种跑步目标（热身、慢跑、冲刺等）
- 歌曲情绪AI分析：AI分析情绪、能量、氛围，可视化情绪雷达图
- 音乐知识库：AI提供歌曲背景、创作故事、制作信息、文化影响
- 歌手关系图谱：构建合作关系图谱，分析风格相似度和社群聚类
- 智能歌单修复：检测并修复重复、缺失信息、低质量等问题歌曲
- 音乐知识问答：4种题型、4种难度的问答游戏，附带排行榜

### 修复

- 修复 musicQuiz.ts 未使用的 chat 导入
- 修复 runningMode.ts 访问已置 null 变量和运算优先级 bug

---

## [1.9.0](https://github.com/ZA12A230/lx-netease-music-mobile/compare/v1.8.85...v1.9.0) - 2026-07-27

### 新增 - 开发者模式

- 调试面板、性能监控、日志查看、网络检查器
- 调试快照生成、网络测试工具
- 6个调试选项开关（性能浮层、日志浮层、详细日志、网络检查器、崩溃报告、调试模式）
...
_为了防止歌曲缓存被第三方软件当做垃圾意外清理，歌曲缓存不再存储到缓存目录。若想清理缓存，需去「设置 → 其他 → 资源缓存管理」清理。_

_更新到该版本后首次播放歌曲时，会将之前的歌曲缓存迁移到新位置，需要等待的时间取决于你已缓存资源的大小。_
