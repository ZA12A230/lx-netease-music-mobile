import { httpFetch } from '../../request'
import settingState from '@/store/setting/state'
import { toast } from '@/utils/tools'

/**
 * 酷狗音乐用户API模块
 * 通过Cookie获取用户信息、收藏歌单、喜欢歌曲等
 */
export default {
  /**
   * 获取用户UID
   */
  async getUid(cookie, retryNum = 0) {
    if (!cookie) throw new Error('Cookie is required to get UID')
    const maxRetries = 3
    const retryDelay = 200

    try {
      const request = httpFetch('https://m.kugou.com/api/v2/user/info', {
        method: 'get',
        headers: {
          'User-Agent': 'Mozilla/5.0 (Linux; Android 13; Pixel 7) AppleWebKit/537.36',
          Referer: 'https://m.kugou.com',
          cookie,
        },
      })
      const { body, statusCode } = await request.promise
      if (statusCode !== 200 || body.status !== 1) throw new Error('获取酷狗UID失败')
      const uid = body.data?.userid
      if (!uid) throw new Error('登录已过期或Cookie无效')
      return String(uid)
    } catch (error) {
      if (retryNum < maxRetries) {
        await new Promise(resolve => setTimeout(resolve, retryDelay))
        return this.getUid(cookie, retryNum + 1)
      }
      throw error
    }
  },

  /**
   * 获取用户收藏歌单
   */
  async getUserPlaylists(uid, cookie, retryNum = 0) {
    const maxRetries = 3
    const retryDelay = 200
    try {
      const request = httpFetch('https://m.kugou.com/api/v2/special/getlist', {
        method: 'get',
        headers: {
          'User-Agent': 'Mozilla/5.0 (Linux; Android 13; Pixel 7) AppleWebKit/537.36',
          Referer: 'https://m.kugou.com',
          cookie,
        },
        form: {
          userid: uid,
          page: 1,
          pagesize: 100,
        },
      })
      const { body, statusCode } = await request.promise
      if (statusCode !== 200 || body.status !== 1) throw new Error('获取酷狗歌单失败')
      return (body.data?.list || []).map(item => ({
        id: item.specialid,
        name: item.specialname,
        coverImgUrl: item.imgurl || '',
        userId: uid,
        trackCount: item.songcount || 0,
      }))
    } catch (error) {
      if (retryNum < maxRetries) {
        await new Promise(resolve => setTimeout(resolve, retryDelay))
        return this.getUserPlaylists(uid, cookie, retryNum + 1)
      }
      throw error
    }
  },

  /**
   * 获取收藏的歌手列表
   */
  async getSublist(cookie, retryNum = 0) {
    const maxRetries = 3
    const retryDelay = 200
    try {
      const request = httpFetch('https://m.kugou.com/api/v2/singer/sublist', {
        method: 'get',
        headers: {
          'User-Agent': 'Mozilla/5.0 (Linux; Android 13; Pixel 7) AppleWebKit/537.36',
          Referer: 'https://m.kugou.com',
          cookie,
        },
        form: { page: 1, pagesize: 200 },
      })
      const { body, statusCode } = await request.promise
      if (statusCode !== 200 || body.status !== 1) throw new Error('获取酷狗关注歌手失败')
      return (body.data?.list || []).map(s => ({
        id: s.singerid,
        name: s.singername,
        picUrl: s.imgurl || '',
        alias: null,
        albumSize: s.albumcount || 0,
        img1v1Url: s.imgurl || '',
      }))
    } catch (error) {
      if (retryNum < maxRetries) {
        await new Promise(resolve => setTimeout(resolve, retryDelay))
        return this.getSublist(cookie, retryNum + 1)
      }
      throw error
    }
  },

  /**
   * 获取收藏的专辑列表
   */
  async getAlbumSublist(cookie, retryNum = 0) {
    const maxRetries = 3
    const retryDelay = 200
    try {
      const request = httpFetch('https://m.kugou.com/api/v2/album/sublist', {
        method: 'get',
        headers: {
          'User-Agent': 'Mozilla/5.0 (Linux; Android 13; Pixel 7) AppleWebKit/537.36',
          Referer: 'https://m.kugou.com',
          cookie,
        },
        form: { page: 1, pagesize: 200 },
      })
      const { body, statusCode } = await request.promise
      if (statusCode !== 200 || body.status !== 1) throw new Error('获取酷狗收藏专辑失败')
      return (body.data?.list || []).map(a => ({
        id: a.albumid,
        name: a.albumname,
        picUrl: a.imgurl || '',
        artists: [{ id: a.singerid, name: a.singername }],
        publishTime: 0,
        size: a.songcount || 0,
      }))
    } catch (error) {
      if (retryNum < maxRetries) {
        await new Promise(resolve => setTimeout(resolve, retryDelay))
        return this.getAlbumSublist(cookie, retryNum + 1)
      }
      throw error
    }
  },

  /**
   * 验证Cookie有效性
   */
  async checkCookie(cookie) {
    try {
      await this.getUid(cookie)
      return true
    } catch {
      return false
    }
  },
}